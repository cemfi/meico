...
public class TempoDef extends AbstractDef {
     ...
     */
    private TempoDef(String name, double value) throws InvalidDataException {
     ...
     * @throws InvalidDataException
     */
    private TempoDef(Element xml) throws InvalidDataException {
        this.parseData(xml);
    }
    ...
    protected void parseData(Element xml) throws InvalidDataException {
        if (xml == null)
            throw new InvalidDataException("Cannot generate TempoDef object. XML Element is null.");

        // parse the dynamicsDef element
        this.name = Helper.getAttribute("name", xml);                                           // get its name attribute
        if (this.name == null) {                                                        // if no name
            throw new InvalidDataException("Cannot generate TempoDef object. Missing name attribute.");
        }
        Attribute value = Helper.getAttribute("value", xml);                                    // get its value attribute
        if (value == null) {                                                            // if no value
            throw new InvalidDataException("Cannot generate TempoDef object. Missing value attribute.");
        }
